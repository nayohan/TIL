...
for deg = 0:2: noRev * 360
    ...
    if(deg<180)
        ...
        orbit2 = [orbit2; y(1,6) y(2,6) y(3,6)];
        [m,n] = size(orbit);
        if(m<=30)
            line(orbit(:,1), orbit(:,2), orbit(:,3), 'LineWidth', 2,'Color',[0 0 1]);
            line(orbit2(:,1), orbit2(:,2), orbit2(:,3), 'LineWidth', 2,'Color',[1 0 0]);    
        else
            for index = 1:30
                line(orbit([m-index m-index+1],1), orbit([m-index m-index+1],2), orbit([m-index m-index+1],3), 'LineWidth',2,'Color',[index/30 index/30 1]);
                line(orbit2([m-index m-index+1],1), orbit2([m-index m-index+1],2), orbit2([m-index m-index+1],3), 'LineWidth',2,'Color',[1 index/30 index/30]);
            end
        end
        for i= 1:1:8
